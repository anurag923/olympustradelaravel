...
class UserController extends Controller
{
    ...
    public function finalbet(Request $request){
        $id = $request->bet_id;
        $res = Userfinalbet::where('id',$id)->update(['end_date'=>date("Y-m-d"),'end_time'=>date("H:i:s"),'profitloss'=>$request->profitloss]);
        // $data->user_id = auth()->user()->id;
    ...
    public function viewallfinalbets(){
        $finalbets = Userfinalbet::with(['user'=>function($q){
            $q->distinct()->select(['id','name','email']);
        }])->with(['duration'=>function($q){
            $q->distinct()->select(['id','timer','payout']);
        }])->distinct()->select(['id','user_id','market','betamount','start_date','start_time','timer_id','end_date','end_time','profitloss','active_bet_id'])->
        addSelect(['exposure'=>Useractivebet::select('exposure')->whereColumn('id','userfinalbets.active_bet_id')])->get();
    ...
    public function allusers(){
        $users = User::distinct()->select(['id','name','email'])->get();
    ...
    public static function liverate(){
        $response = Http::get('https://api.polygon.io/v1/open-close/crypto/BTC/USD/2021-09-24?adjusted=true&apiKey=6sEFcNe2upitHW5lt9dp7EfkIuxoR58k');
        $val = $response->json();
        $close = $val['close'];
